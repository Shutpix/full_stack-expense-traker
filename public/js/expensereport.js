...
async function renderList()
{
    ...
    const response = await axios.get("http://65.1.136.178:3000/getexpenses",{ headers: { Authorization: token }});
...
download.addEventListener('click', async (e)=>{
    e.preventDefault();
    const token = localStorage.getItem("token");
    const response = await axios.get("http://65.1.136.178:3000/download",{headers : { Authorization: token }})
